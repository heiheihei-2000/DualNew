...
class DataLoader:
    ...
    def get_neighbors(self, nodes, qids=None, device=None):
        ...
        if 'webqsp' in self.task_dir or 'CWQ' in self.task_dir:
            n_ent = len(nodes)
            KG = np.concatenate(
                [nodes[:, 1].reshape(-1, 1), 2 * self.n_rel * np.ones((n_ent, 1)), nodes[:, 1].reshape(-1, 1)], 1)
            for qid in qids:
                skg = self.kgs[qid]
    ...
    def get_batch(self, batch_idx, data='train'):
        ...
        if data == 'valid':
            query, topic, answer = np.array(self.valid_q), np.array(self.valid_e, dtype=object), np.array(self.valid_a,
                                                                                                          dtype=object)
        if data == 'test':
            query, topic, answer = np.array(self.test_q), np.array(self.test_e, dtype=object), np.array(self.test_a,
                                                                                                        dtype=object)
