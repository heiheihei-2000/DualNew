...
from utils.utils import cal_ranks, cal_performance, cal_top1
...
class BaseModel(object):
    ...
    def get_path(self, mode='test', dataset='webqsp'):
        ...
        n = 10
        n_batch = n_data // batch_size + (n_data % batch_size > 0)
        f = open(dataset+'-'+mode+'-'+'path.txt', 'w')
        self.model.eval()
        for i in tqdm(range(n_batch)):
            start = i * batch_size
            end = min(n_data, (i + 1) * batch_size)
            ...
            self.model.visual_path(subs, rels, objs, filepath=dataset+'-'+mode+'-'+'path.txt', mode=mode)

        print('path generate done in ' + dataset+'-'+mode+'-'+'path.txt' + '\n')
