...
if __name__ == '__main__':
    ...
    if dataset == 'MetaQA/1-hop':
        ...
        opts.n_tbatch = 20
        opts.K = 200
        loaders = [DataLoader('MetaQA/1-hop'), DataLoader('MetaQA/2-hop'), DataLoader('MetaQA/3-hop'),
                   DataLoader('webqsp_nsm'), DataLoader('CWQ')]

    opts.sample = 1
    opts.n_ent = loaders[0].n_ent
    opts.n_rel = loaders[0].n_rel

    config_str = '%d, %d, %.6f, %.4f, %.6f,  %d, %d, %d, %d, %.4f,%s\n' % (opts.sample, opts.K, opts.lr,
                                                                           opts.decay_rate, opts.lamb, opts.hidden_dim,
                                                                           opts.attn_dim, opts.n_layer, opts.n_batch,
    ...
    for epoch in range(40):
        ...
        for i in range(len(loaders)):
            ...
            if h1 > epoch_h1:
                epoch_h1 = h1
                epoch_out_str = out_str
        
        # 保存全局最佳h1分数的模型
        if epoch_h1 > best_h1:
            best_h1 = epoch_h1
            best_str = epoch_out_str
            print(f'New best H@1: {best_h1:.4f} at epoch {epoch}')
            
            # 保存最佳模型
            dataset_name = dataset.replace('/', '-')
            model.save_model(dataset_name + '_best', out_str=f'hit@1:{best_h1:.4f}')
        ...
        if dataset == 'WebCWQ' and epoch == 19:
            model.save_model(dataset, out_str=f'hit@1:{epoch_h1}')
            break
